...
Resume-правило: если каталог уже существует, не начинай заново. `feature-brief.md` означает fast-path; `prd.md`/полный набор — полный трек. Продолжай с первого обязательного артефакта, который не достиг `Approved`. Если плановый документ уже `Approved`, а в `memory/decisions.md` нет указателя на slug, Stage 6 оборвана: продолжай Handoff. Если набор неоднозначен, прочитай историю `intake.md` и попроси решение только если она не разрешает конфликт.
...
- В `memory/decisions.md` добавляется одна строка-указатель на утверждённую фичу.
  ...
  сохранён и сообщён. Плановый документ `Approved` без указателя означает оборванный
  Handoff и требует resume со Stage 6.

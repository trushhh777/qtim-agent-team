...
и заканчивается секцией `## История изменений` (append-only, строка на ревизию или зафиксированное в реализации отклонение от плана).
...
- Тот, кто реализует, обновляет Status артефактов (`In Development` при старте, `Done` после прохождения gates) и фиксирует отклонения от плана с обоснованием и всплывшие edge cases строкой в «Истории изменений» `plan.md` — это вход для ретроспективы эпика и resume многофазных фич.

...
3. **Агенты.** Для каждой роли из charter (кроме встроенных `explorer`/`worker` — им свой файл не требуется): `.codex/agents/<name>.toml` существует; TOML парсится (`tomllib` при наличии Python 3.11+); `name` в TOML совпадает с именем агента из charter; первой строкой `# qtim-version: ...`; в теле нет неподставленных плейсхолдеров `{{...}}` (остались -> генерация не подставила стек: `$qtim-update` или поправить руками); нет plugin-internal путей (`../../...`) — сгенерированные файлы должны быть самодостаточны.
